		...
		<form name="form_slide_params" id="form_slide_params" class="slide-main-settings-form">
			...
			if(!$slide->isStaticSlide()){
				...
				<div id="slide-main-image-settings-content" class="slide-main-settings-form">
					...
					<ul class="rs-layer-main-image-tabs" style="display:inline-block;margin-top: 15px; ">
					...
					<span id="mainbg-sub-source" style="display:block">
						...
						<span class="diblock bg-settings-block">
							...
							if($slider_type == 'posts' || $slider_type == 'specific_posts' || $slider_type == 'product'){ 
								?>
							...
							<span id="tp-bgimagewpsrc" class="bgsrcchanger-div" style="display:none;margin-left:20px;">																
								<a href="javascript:void(0)" id="button_change_image" class="button-primary revblue" ><i class="prestashop-icon"></i><?php _e("Media Library", 'revslider'); ?></a>
					...
					<div id="mainbg-sub-setting" style="display:none">
						...
						<div id="video-settings" style="display: block;">
							...
							<div class="vid-rev-vimeo-youtube video_volume_wrapper">
								...
								<input type="text" id="video_volume" name="video_volume" <?php echo $video_volume; ?>>
							</div>
							...
							<span id="vid-rev-youtube-options">
								...
								<div>
									...
									<input type="text" id="video_arguments" name="video_arguments" style="width:350px;" value="<?php echo $video_arguments; ?>">
								</div>
							</span>
							<div id="vid-rev-vimeo-options">
								<label><?php _e('Arguments Vimeo:', 'revslider'); ?></label>
								<input type="text" id="video_arguments_vim" name="video_arguments_vim" style="width:350px;" value="<?php echo $video_arguments_vim; ?>">
							</div>
						</div>
					...
					<div style="margin-top:10px">
						...
						<div style="display:inline-block; vertical-align: top;">
							...
							if(intval($slide_thumb) > 0){
								?>
								<div style="width:100px;height:70px;background:url('<?php echo RevLoader::getConstants('ajaxurl'); ?>?action=revslider_show_image&amp;img=<?php echo $slide_thumb; ?>&amp;w=100&amp;h=70&amp;t=exact'); background-position:center center; background-size:cover;"></div>
								<?php
							}elseif($slide_thumb !== ''){
								?>
												...
												switch($placeholder['type']){
													...
													case 'font-family':
														...
														foreach($font_families as $handle => $name){
															...
															if($ph_vals['font_family'] == $name['label']){
																echo ' selected="selected"';
															}
															echo '>'. $name['label'] .'</option>';													
														}
														?>
														</select>												
														<?php
			...
			<div id="slide-addon-wrapper" style="margin:-15px; display:none">
					...
					if(!empty($slide_general_addon)){
						foreach($slide_general_addon as $rs_addon_handle => $rs_addon){
							...
							<span class="rs-layer-toolbar-box">
								<span id="rs-addon-settings-trigger-<?php echo $rs_addon_handle; ?>" class="rs-addon-settings-trigger"><?php echo $rs_addon['title']; ?></span>
							</span>
							<?php
						}
					}
				...
				<div style="border-top:1px solid #ddd;">
					...
					if(!empty($slide_general_addon)){
						foreach($slide_general_addon as $rs_addon_handle => $rs_addon){
							?>
							<div id="rs-addon-settings-trigger-<?php echo $rs_addon_handle; ?>-settings" class="rs-addon-settings-wrapper-settings" style="display: none;">
				...
				<div id="slide-info-settings-content" style="display:none">
					<ul>
						...
						for($i=1;$i<=10;$i++){
							?>
							<li>
								<label><?php _e('Parameter', 'revslider'); echo ' '.$i; ?></label> <input type="text" name="params_<?php echo $i; ?>" value="<?php echo stripslashes(RevSliderFunctions::getVal($slideParams, 'params_'.$i,'')); ?>">
								<?php _e('Max. Chars', 'revslider'); ?> <input type="text" style="width: 50px; min-width: 50px;" name="params_<?php echo $i; ?>_chars" value="<?php echo RevSliderFunctions::getVal($slideParams, 'params_'.$i.'_chars',10, RevSlider::FORCE_NUMERIC); ?>">
