...
class RevSliderObjectLibrary {
	...
	public function _get_object_data($object_handle){
		...
		//$upload_dir = wp_upload_dir(); // Set upload folder
                ...
                $upload_dir['basedir'] = wp_upload_dir();
                $upload_dir['baseurl'] = wp_upload_url();
	...
	public function _is_object($url){
		...
		$upload_dir				  = wp_upload_dir(); // Set upload folder
                $upload_url				  = wp_upload_url(); // Set upload folder
                $upload_directory         = $upload_dir . $this->object_orig_path;
		//$upload_directory         = $upload_dir['basedir'] . $this->object_orig_path;
		$upload_url         	  = $upload_url . $this->object_orig_path;
			...
			function createFontIcons() {
				...
				if (sheets)
					jQuery.each(sheets,function(index,sheet) {
						...
						var found = false,
							markup = "";	
						if (sheet && sheet.href && sheet.href.indexOf("public/assets/fonts/")>=0) {							
							try{
		...
		if( $imgsize !== false) {
			...
			foreach($sizes as $size){
				...
				if(!file_exists($upload_directory.$modified_file_name_without_ending.'.'.$file_ending) || $force) {
					...
					$newfilename = $modified_file_name_without_ending.'.'.$file_ending;
//					$image->resize($width, $height);
//					$image->save($upload_directory.$modified_file_name_without_ending.'.'.$file_ending);
//				
                                        ImageManager::resize($upload_directory.$file_name_with_ending, $upload_directory.$newfilename, $width, $height);
                                        //$res_img->resize($upload_directory.$file_name_with_ending,$upload_directory.$newfilename, $width, $height);
                                }
			}
	...
	public function retrieve_all_object_data(){
		...
		$obj['online'] = json_decode($obj['online'],true);
