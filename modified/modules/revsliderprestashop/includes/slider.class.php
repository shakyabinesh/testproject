...
//        01911304120
	...
	public function initByID($sliderID){
		...
		RevSliderGlobals::$table_sliders =DB_PREFIX.RevSliderGlobals::TABLE_SLIDERS_NAME;
		try{
	...
	private function deleteAllSlides(){
		$this->validateInited();
		RevSliderGlobals::$table_slides =DB_PREFIX.RevSliderGlobals::TABLE_SLIDES_NAME;
	...
	public function deleteStaticSlide(){
		$this->validateInited();
		RevSliderGlobals::$table_static_slides =DB_PREFIX.RevSliderGlobals::TABLE_STATIC_SLIDES_NAME;
	 ...
	 */
		...
		if(class_exists('ZipArchive')){
			...
			$zip = new ZipArchive();
                    //    var_dump(GlobalsRevSlider::$uploadsUrlExportZip);die();
...
//		}
		...
		//add images to zip
           //     var_dump($usedImages);die();
 		if(!empty($usedImages)){
			$upload_dir = RevSliderFunctionsWP::getPathUploads();
                        $upload_dir_multisiteless = wp_upload_dir();
			$upload_dir_multisiteless_baseurl= wp_upload_url();
                        
			$cont_url = $upload_dir_multisiteless_baseurl;
                        
			$cont_url_no_www = str_replace('www.', '', $upload_dir_multisiteless_baseurl);  
		...
		//var_dump(PCLZIP_ATT_FILE_NAME);die("dfdf");
		...
		$real_slider_id = '';
		 try{
			...
			if(!class_exists("ZipArchive")){
					...
					$unzipfile = $zip->open($filepath, ZIPARCHIVE::CREATE); 
                                        
                                        $zip->extractTo($rem_path); 
				}
//		 
                                $content='';
			if( $unzipfile ){
				...
				//$content = ( $wp_filesystem->exists( $d_path.'slider_export.txt' ) ) ? $wp_filesystem->get_contents( $d_path.'slider_export.txt' ) : '';
                                ...
                                if(is_numeric($zip->locateName('slider_export.txt'))){$content = $zip->getFromName('slider_export.txt');}
                                
                                if($content == ''){
                                    RevSliderFunctions::throwError(__('slider_export.txt does not exist!', 'revslider'));
				} 
                        
                                 if(is_numeric($zip->locateName('custom_animations.txt'))){$animations = $zip->getFromName('custom_animations.txt');}else $animations = '';
                                 if(is_numeric($zip->locateName('dynamic-captions.css'))){$dynamic = $zip->getFromName('dynamic-captions.css');}else $dynamic = '';
                                 if(is_numeric($zip->locateName('static-captions.css'))){$static = $zip->getFromName('static-captions.css');}else $static = '';
                                 if(is_numeric($zip->locateName('navigation.txt'))){$navigations = $zip->getFromName('navigation.txt');}else $navigations = '';
                                 if(is_numeric($zip->locateName('info.cfg'))){$uid_check = $zip->getFromName('info.cfg');}else $uid_check = '';
                                 if(is_numeric($zip->locateName('version.cfg'))){$version_check = $zip->getFromName('version.cfg');}else $version_check = '';
				...
				if($importZip === true){ //we have a zip, check if exists
					...
					if(isset($params["image"])){
						...
						$params["image"] = RevSliderBase::check_file_in_zip($d_path, $params["image"], $sliderParams["alias"], $alreadyImported);
                                                ...
                                                $params["image"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($params["image"]);
					...
					if(isset($params["background_image"])){
						...
						$params["background_image"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($params["background_image"]);
					...
					if(isset($params["slide_thumb"])){
						...
						$params["slide_thumb"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($params["slide_thumb"]);
					...
					if(isset($params["show_alternate_image"])){
						...
						$params["show_alternate_image"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($params["show_alternate_image"]);
					}
					if(isset($params['background_type']) && $params['background_type'] == 'html5'){
						if(isset($params['slide_bg_html_mpeg']) && $params['slide_bg_html_mpeg'] != ''){
							$params['slide_bg_html_mpeg'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_mpeg"], $sliderParams["alias"], $alreadyImported, true));
						}
						if(isset($params['slide_bg_html_webm']) && $params['slide_bg_html_webm'] != ''){
							$params['slide_bg_html_webm'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_webm"], $sliderParams["alias"], $alreadyImported, true));
						}
						if(isset($params['slide_bg_html_ogv'])  && $params['slide_bg_html_ogv'] != ''){
							$params['slide_bg_html_ogv'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $params["slide_bg_html_ogv"], $sliderParams["alias"], $alreadyImported, true));
						}
					}
				}
				...
				foreach($layers as $key=>$layer){					
					...
					if($importZip === true){ //we have a zip, check if exists
						if(isset($layer["image_url"])){
							...
							$layer["image_url"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($layer["image_url"]);
						}
						if(isset($layer["bgimage_url"])){
							...
							$layer["bgimage_url"] = RevSliderFunctionsWP::getImageUrlFromPathForImport($layer["bgimage_url"]);
						}
						if(isset($layer['type']) && ($layer['type'] == 'video' || $layer['type'] == 'audio')){
							...
							if(!empty($video_data) && isset($video_data['video_type']) && $video_data['video_type'] == 'html5'){
								...
								if(isset($video_data['urlPoster']) && $video_data['urlPoster'] != ''){
									$video_data['urlPoster'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["urlPoster"], $sliderParams["alias"], $alreadyImported));
								}
								
								if(isset($video_data['urlMp4']) && $video_data['urlMp4'] != ''){
									$video_data['urlMp4'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["urlMp4"], $sliderParams["alias"], $alreadyImported, true));
								}
								if(isset($video_data['urlWebm']) && $video_data['urlWebm'] != ''){
									$video_data['urlWebm'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["urlWebm"], $sliderParams["alias"], $alreadyImported, true));
								}
								if(isset($video_data['urlOgv']) && $video_data['urlOgv'] != ''){
									$video_data['urlOgv'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["urlOgv"], $sliderParams["alias"], $alreadyImported, true));
								}
								
							}elseif(!empty($video_data) && isset($video_data['video_type']) && $video_data['video_type'] != 'html5'){ //video cover image
								if($video_data['video_type'] == 'audio'){
									if(isset($video_data['urlAudio']) && $video_data['urlAudio'] != ''){
										$video_data['urlAudio'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["urlAudio"], $sliderParams["alias"], $alreadyImported, true));
									}
								}else{
									if(isset($video_data['previewimage']) && $video_data['previewimage'] != ''){
										$video_data['previewimage'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $video_data["previewimage"], $sliderParams["alias"], $alreadyImported));
									}
								}
							}
							...
							if(isset($layer['video_image_url']) && $layer['video_image_url'] != ''){
								$layer['video_image_url'] = RevSliderFunctionsWP::getImageUrlFromPathForImport(RevSliderBase::check_file_in_zip($d_path, $layer["video_image_url"], $sliderParams["alias"], $alreadyImported));
							}
						}
			...
			if(!empty($slider_map)){
				foreach($arrSlides as $sl_key => $slide){
					...
					foreach($slide['layers'] as $key => $value){
                      ...
                      //  var_dump($arrSlider['static_slides']);
...
//                                                                                  //   var_dump($sliderParams["alias"]);die();
							...
							if(trim($layer["bgimage_url"]) !== ''){
								if(strpos($layer["bgimage_url"], 'http') !== false){
									...
									if($importZip === true){ //we have a zip, check if exists
										...
										$bgimage_url = $zip->getStream('images/'.$layer["bgimage_url"]);
                                                                                
                                                                                if(!$bgimage_url){
											echo $layer["bgimage_url"].__(' not found!<br>');
										}else{
//											if(!isset($alreadyImported['zip://'.$filepath."#".'images/'.$layer["bgimage_url"]])){
//												$importImage = RevSliderFunctionsWP::import_media('zip://'.$filepath."#".'images/'.$layer["bgimage_url"], $sliderParams["alias"].'/');
//												
//												if($importImage !== false){
//													$alreadyImported['zip://'.$filepath."#".'images/'.$layer["bgimage_url"]] = $importImage['path'];
//													
//													$layer["bgimage_url"] = $importImage['path'];
//												}
//											}else{
//												$layer["bgimage_url"] = $alreadyImported['zip://'.$filepath."#".'images/'.$layer["bgimage_url"]];
//											}
                                                                                    $layer["bgimage_url"] = RevSliderBase::check_file_in_zip($d_path, $layer["bgimage_url"], $sliderParams["alias"], $alreadyImported);
					...
					$arrCreate["params"] = $my_params;
					$arrCreate["settings"] = $my_settings;
					//var_dump(json_decode($my_layers,true)); 
					if($sliderExists){
...
//				}
         ...
         public function getSliderImgSettings($sliderID)
    {
        ...
        $sliderID = $this->db->realEscape($sliderID);
	...
	public function getSlidesFromStream($publishedOnly = false){
		...
		switch($sourceType){
			...
			case 'youtube':
				...
				//var_dump($this->getParam('youtube-playlist'));die();
	...
	public function getFirstSlideIdFromGallery(){
		...
		$arrSlideRecords = $this->db->fetch(RevSliderGlobals::$table_slides,$this->db->prepare("slider_id = %s", array($this->id)),"slide_order",'',' LIMIT 0,1');
                ...
                //var_dump(RevSliderGlobals::$table_slides);die();
	...
	public function getSlidesForOutput($publishedOnly = false, $lang = 'all',$gal_ids = array()){
		...
		if(RevSliderWpml::isWpmlExists() && $this->getParam('use_wpml', 'off') == 'on'){
			...
			//$wpml_current_language = apply_filters( 'wpml_current_language', null );
                        $wpml_current_language = RevSliderWpml::getCurrentLang();
...
//		}
	...
	private function getProductsFromCategories($publishedOnly = false){
		...
		$catIDs = $this->getParam("product_category");
              ...
              //  $catIDs = 'product_cat_20,product_cat_24,product_cat_33';
               //  var_dump($catIDs);die();
		$data = RevSliderFunctionsWP::getCatAndTaxData($catIDs);
		...
		$arrAddition = array_merge($arrAddition, array());
			...
			$catIDs = array($catIDs);
               ...
               // var_dump($catIDs);die();
                foreach($catIDs as $catID){
                    ...
                    $getProducts = sdsconfig::getProducts($data);
